...
class TaskAchieveModel extends BaseModel {
     ...
     * @return integer 结果
     */
    public static function getUserInfoByCondition( $uid,$condition,$pdo = null )
    {
        if(is_null($pdo))
        {
            $pdo = Database::getPdo();
        ...
        $stmt = $pdo->prepare("SELECT $condition FROM user_info where n_id = :uid ");
        
        $stmt->bindValue(":uid" , $uid);
        $stmt->execute();
        $user_info = self::fetchToArray( $stmt );
        $user_info=isset($user_info[0])?$user_info[0]:$user_info;
        $arr = unserialize($user_info[$condition]);
        return $arr;
    }
